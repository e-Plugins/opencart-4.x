...
{
    // Constants
    const APP_ID = "dw_opencart4.x_1.0.6";
    ...
    const ERR_NO_RTLO = "Geen Outletcode bekend; controleer de module instellingen |
        No Outletcode filled in, check the module settings";
    ...
    protected $bankwireAmountDue = 0;
    protected $bankwireAmountPaid = 0;
    ...
    {
        $additionalParam1 = $additionalParam2 = '';
        $_result = explode("|", $strResult);
        if(count($_result)  == 1) {
            @list($resultCode) = $_result;
        }
        if(count($_result)  == 2) {
            @list($resultCode, $additionalParam1) = $_result;
        }
        if(count($_result)  == 3) {
